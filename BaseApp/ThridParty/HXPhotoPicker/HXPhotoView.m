...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.tempShowAddCell) {
        if (indexPath.item == self.dataList.count) {
            ...
            if (self.cellCornerRadius) {
                addCell.cellCornerRadius = self.cellCornerRadius;
            }
            if (self.cellBackgroundColor) {
                addCell.cellBackgroundColor = self.cellBackgroundColor;
            }
            return addCell;
        }
    }
    ...
    if (self.cellCornerRadius) {
        cell.cellCornerRadius = self.cellCornerRadius;
    }
    if (self.cellBackgroundColor) {
...
- (void)setupNewFrame {
    ...
    CGFloat itemW = ([UIScreen mainScreen].bounds.size.width - 2 * self.edgeSpacing - self.spacing * (self.lineCount - 1)) / self.lineCount;
